...
export async function copyTextContentToClipboard(callElem) {
  ...
  if(copyTarget !== undefined)
    navigator.clipboard.write(copyTarget.textContent)
  else
...
export async function mixinRemotePagelet(callElem) {
  ...
  if(callElem.hasAttribute('framed'))
    pageletElem = await controllerFrameAround(await getRemotePagelet(pageletSrc))
  else
    pageletElem = await getRemotePagelet(pageletSrc)
...
export async function mixinRemotePageletFramed(callElem) {
  ...
  let pageletElem = await controllerFrameAround(await getRemotePagelet(pageletSrc))
...
export async function copyToClipboard(callElem) {
  ...
  // else
  navigator.clipboard?.write(callElem.dataset.payload)
  lib.notificationFrom(callElem, `Due to certain bizarre design decisions in the clipboard API, copying to the clipboard is untested. Sorry if it doesn't work!`, {transient: true})
}