...
const dropdownContainerTemplate = document.createElement('template')
dropdownContainerTemplate.innerHTML = /*html*/`
...
export async function makeDropdown(callElem) {
  ...
  let existingContainer = document.body.querySelector(`.dropdown-root[data-origin="${id}"]`)
  if(existingContainer)
    return void existingContainer.remove()
  // else
  
  const dropdownContainer = dropdownContainerTemplate.content.cloneNode(true).firstElementChild
  dropdownContainer.dataset['origin'] = id
  const dropdown = callElem.querySelector('template').content.cloneNode(true).firstElementChild
  dropdownContainer.querySelector('.dropdown-relative').appendChild(dropdown)
  
  let callElemRect = callElem.getBoundingClientRect()
  dropdownContainer.style.left = `${window.visualViewport.pageLeft + callElemRect.left}px`
  dropdownContainer.style.top  = `${window.visualViewport.pageTop  + callElemRect.top}px`
  // let rect = dropdownContainer.querySelector('.dropdown-relative').getBoundingClientRect()
  ...
  if(dropdown.hasAttribute('remove-on-click')) {
    document.addEventListener('click', () => setTimeout(()=>dropdownContainer.remove()), {once: true})
  } else if(dropdown.hasAttribute('remove-on-external-click')) {
    const removeFn = (clickEvent) => {
      if(!dropdownContainer.contains(clickEvent.target)) {
        setTimeout(() => {
          dropdownContainer.remove()
          document.removeEventListener('click', removeFn)
        })
      }
    }
    document.addEventListener('click', removeFn)
  }
