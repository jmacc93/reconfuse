...
export function notificationFrom(elem, message, opts = {}) {
  ...
  if(dropdownParent) // is in dropdown, jump to dropdown origin
    messageSource = document.body.querySelector(`#${dropdownParent.dataset.origin}`) ?? elem // default to elem if cant find origin
...
// export function getOrigin(startElem) {
//   const originid = startElem.closest('*[data-origin]')?.getAttribute('data-origin')
//   if(!originid)
//     return undefined
//   // else
//   return document.body.querySelector(`#${originid}`)
// }
...
export async function openPageletAt(elem, url, compoundSelector) {
  ...
  // else
  if(targetSelector === undefined) writeAndThrow('No target selector given for target attribute', this)
  // else
  let targetElem = selectTargetElement(elem, targetSelector)
  if(!targetElem) writeAndThrow(`openPageletAt target element not found using selector ${targetSelector}, method ${method}`, this)
  // else
  ...
  try {
    switch(splitMethod[0]) {
      case 'append':  targetElem.appendChild(newElem); break
      case 'replace': 
        styleInProgress(targetElem)
        targetElem.replaceWith(newElem)
        break
      case 'after':   targetElem.after(newElem); break
      default: writeAndThrow(`openPageletAt unknown method ${splitMethod[0]}`, this)
    }
  } catch(err) {
