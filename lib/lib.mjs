...
export function applyOnNewChildren(elem, fn, deep = false) {
  ...
  observer.observe(elem, {childList: true, substree: deep})
}
...
*/
export function getFirstInTreeMatching(elem, selector) {
...
export function selectTargetElement(elem, code) {
  ...
  for(const codelet of splitCode) {
...
export function makePageletFromSource(src) {
  ...
  pageletElem.innerHTML = src
  pageletElem = pageletElem.content
  replicateAndReplaceScripts(pageletElem)
...
export async function getRemotePagelet(src, errorMode = ERRORONNOTOK) {
  ...
  return makePageletFromSource(pageletBody)
}
...
export const stdTextExpansions = {
  ...
  ["!MLREP"]:         {to: ()=> "/pagelets/represent-message-list.jhp?channel="     , endPosition: -1},
  ["!RAND"]:          {to: ()=> randomTokenString(6)                                , endPosition: -1},
  
  // Markdown
  ["!MDNEWINSTLINK"]: {to: ()=> `[](/pagelets/represent-file.jhp?file=/instances/)` , endPosition: -2},
  ["!MDLINK"]:        {to: ()=> "[]()"                                              , endPosition: -2},
  ["!MDIMG"]:         {to: ()=> "![]()"                                             , endPosition: -2},
  ["!MDREPLINK"]:     {to: ()=> "[](/pagelets/represent-file.jhp?file=)"            , endPosition: -2},
  ["!MDREPIMG"]:      {to: ()=> "![](/pagelets/represent-file.jhp?file=)"           , endPosition: -2},
  
  // Escm
  ["!ESCLINK"]:       {to: ()=> "\\link()"                                          , endPosition: -2},
  ["!ESCIMG"]:        {to: ()=> "\\img()"                                           , endPosition: -2},
  ["!ESCREPLINK"]:    {to: ()=> "\\link(/pagelets/represent-file.jhp?file=)"        , endPosition: -2},
  ["!ESCREPIMG"]:     {to: ()=> "\\img(/pagelets/represent-file.jhp?file=)"         , endPosition: -2},
  ["!ESCMIXIN"]:      {to: ()=> "\\img(/pagelets/represent-file.jhp?file=)"         , endPosition: -2}
}

export function makeTextExpander(replacementsObj) {
  let keyRegex = RegExp(Object.keys(replacementsObj).map(x=>['(?:',x,')'].join('')).join('|'), 'g')
  let ret = function(text) { return ret.withOffset(text)[0] }
  ret.withOffset = function(text) {
    let offset = 0
...
export const stdTextExpand = makeTextExpander(stdTextExpansions)
